...
public class AttendanceInfoServiceImpl implements AttendanceInfoService {
	...
	public Map<String, Object> attendanceList(Map<String, Object> inputMap) {
		...
		////주간누적근무시간 추출
		String sDate =(String) ( (List<AttendanceInfo>) map.get("weekAttList") ).get(0).getWeekStartDate();
		String eDate =(String) ( (List<AttendanceInfo>) map.get("weekAttList") ).get(0).getWeekEndDate();
		   ...
		   (wsInfo.getWeekSumWorktimeHour() == wsInfo.getWorktimePerWeek()
			...
			if(wsInfo.getWeekSumWorktimeMinute() > 0) {
				...
				wsInfo.setWeekOverWorktimeHour(
						wsInfo.getWeekSumWorktimeHour()
						- wsInfo.getWorktimePerWeek() - 1);
			}
		...
		////누적 근무시간이 기준 근무시간보다 적은 경우
			...
			wsInfo.setWeekRemainWorktimeHour(wsInfo.getWorktimePerWeek());
		}
