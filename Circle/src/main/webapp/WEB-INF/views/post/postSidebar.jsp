<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/common/menuBar/menuLeftBar.css">
</head>
<body>
	<div>
		<div class="menuLeftBar">
			<div class="menuLeftListTop">게시판</div>
			<div class="menuLeftListBtn"><button class="menuLeftBtn"><a href="#">게시글 작성</a></button></div>
			<div class="menuLeftList"></div>
			<div class="menuLeftListTitle"><a href="#">전체 게시판</a></div>
			<div class="menuLeftList"><a href="#">공지 사항</a></div>
			<div class="menuLeftListTitle"><a href="#">사원 게시판</a></div>
			<div class="menuLeftList"><a href="#">인사 발령</a></div>
			<div class="menuLeftListTitle"><a href="#">전사 게시판</a></div>
			<div class="menuLeftList"><a href="#">인사과</a></div>
			<div class="menuLeftList"><a href="#">홍보부</a></div>															
		</div>
	</div>
</body>
</html>