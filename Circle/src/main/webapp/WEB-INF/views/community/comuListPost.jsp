	...
	<div class="wrap">
		...
		<div class="container">
			...
			<table id="listArea" style="text-align: center; border: 1px solid #FFOODD;">
				...
				<c:forEach var="comuListPost" items="${comuListPost}">
					<tr id="List">
						<td><br>${comuListPost.comu_post_ordr}</td>
						<td><br>${comuListPost.comu_list_name}</td>
						<td><br><a href='<c:url value='/community/comuDetail?comu_post_ordr=${comuListPost.comu_post_ordr}'/>'>${comuListPost.comu_post_title}</a></td>
						<td><br>${comuListPost.emp_info_name}</td>
						<td><br>${comuListPost.comu_post_view}</td>
						<td><br>${comuListPost.comu_post_dat}</td>
				...
				<!-- 현재 페이지 블럭이 1보다 크면 처음 페이지로 이동 -->
					<c:if test="${map2.pager.curBlock > 1}">
					<a href="javascript:list('1')">[처음]</a>
					</c:if>
				<!-- 현재 페이지 블럭이 1보다크면 [이전]을 출력 -->
					<c:if test="${map2.pager2.curBlock > 1}">
					<a href="javascript:list('${map2.pager2.prevPage}')">[이전]</a>
					</c:if>
				<!-- 페이지블럭 처음부터 마지막 블럭까지 1 씩 증가하는 페이지 출력 -->
					<c:forEach var="comu_post_ordr"
					begin="${map2.pager2.blockBegin}" end="${map2.pager2.blockEnd}">
						<c:choose>
							<c:when test="${comu_post_ordr == map2.pager.curPage}">
					...
					<!-- 다음 페이지 블록으로 이동 -->
						<c:if test="${map2.pager2.curBlock <=map2.pager2.totBlock}">
						<a href="javascript:list('${map2.pager2.nextPage}')">[다음]</a>
						</c:if>
					<!-- 끝 페이지로 이동 -->
						<c:if test="${map2.pager2.curBlock <=map2.pager2.totPage}">
						<a style="color:#D9418C;"
							href="javascript:list('${map2.pager2.totPage}')">[끝]</a>			
						</c:if>	
					</td>
				</tr> --%>
